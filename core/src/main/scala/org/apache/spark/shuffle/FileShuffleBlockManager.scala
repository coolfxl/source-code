...
class FileShuffleBlockManager(conf: SparkConf)
   ...
   */
  /**
   * 给每个map task 获取一个ShuffleWriterGroup
   */
  def forMapTask(shuffleId: Int, mapId: Int, numBuckets: Int, serializer: Serializer,
    ...
    new ShuffleWriterGroup {
      ...
      /**
       * 这里就很关键，对应之前所说的，shuffle有两种模式，普通和优化后的模式
       * 那么这里就会进行判断，如果开启了consolidation机制，也就是consolidateShuffleFiles为true的话
      ...
      val writers: Array[BlockObjectWriter] = if (consolidateShuffleFiles) {
        ...
        Array.tabulate[BlockObjectWriter](numBuckets) { bucketId =>
           ...
           * 然后用bucketId,来调用shuffleFileGroup的apply()函数获取一个shufflefilegroup
           */
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)
          /**
           * 然后用blockmanager的getdiskwriter()方法，针对shufflefilegroup获取一个writer
           * 这样的话，如果开启了consolidation机制，实际上，对于每一个bucket，都会获取一个针对shufflefilegroup的writer，而不是一个独立的shuffleblockfile的writer
           * 这样就实现了所谓的多个shufflemaptask的输出数据的合并
           */
         ...
         * 然后调用blockmanager的diskblockmanager，获取一个袋盖哦了要写入的本地磁盘文件的blockfile
         * 而且会判断这个blockfile要是存在的话，还要进行删除，
         * 然后调用blockmanager的getdiskwriter方法，针对那个blockfile，生成writer
         */
        Array.tabulate[BlockObjectWriter](numBuckets) { bucketId =>
          //同样生成一个shuffleblockId
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)
        ...
        /**
         * 总结：所以使用这种普通的shuffle操作的话
         * 对于每一个shufflemaptask输出的bucket
