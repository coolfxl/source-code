  ...
  extends Logging
{
  ...
  def launchTask(
    ...
    //java线程池，executor内部有一个线程池
    //这里其实将task封装在一个线程中，taskrunner，
    //直接静线程丢入线程池，进行执行
    //所以线程池是自多个实现了排队机制的，也就是说，如果线程池内的线程暂时没有空闲的
    //那么丢进去的线程是要排队的
  ...
  class TaskRunner(
    ...
    override def run() {
      ...
      try {
        ...
        //通过网络通信将需要的文件、资源、jar拷贝过来
        updateDependencies(taskFiles, taskJars)
        //最后，通过正式的反序列话操作，静整个task的数据集序列化过来
        /**
         * 为什么要用java的classload②，比如可以用反射的方式来动态加载一个类，然后创建这个类的对象
         * 还有就是可以用于指定上下文的相关资源，进行加载和读取
         */
        ...
        //具体执行操作，这里的value对于shufflemaptask来说就是mapstatus
        //封装了shufflemaptask计算的数据，输出的位置
        //后面，如果还是一个shufflemaptask
        //就会去联系mapoutputtracker,来获取上一个shufflemaptask的输出位置，然后通过网络拉取数据
        ...
        //这里就是对mapstatus进行了各种序列化和封装，因为后面要通过网络发送给driver
        val resultSer = env.serializer.newInstance()
        val beforeSerialization = System.currentTimeMillis()
        val valueBytes = resultSer.serialize(value)
        val afterSerialization = System.currentTimeMillis()

        //这里计算出了task的一些metrics，就是统计信息
        //包括运行了多长时间，反序列化耗费了多少时间，java虚拟机耗费了多长时间，结果的序列化耗费了多长时间
        //sparkUI上可见
        for (m <- task.metrics) {
        ...
        //这个非常核心，其实就是调用了executor所在的coarsegrainedexecutorbackend的statusupdate方法
   ...
   * 
   * 为什么要实现多线程并发访问的同步：因为在这里面访问了诸如：currentfiles等等这些共享的东西
   */
  private def updateDependencies(newFiles: HashMap[String, Long], newJars: HashMap[String, Long]) {
    //获取hadoop配置文件
    lazy val hadoopConf = SparkHadoopUtil.get.newConfiguration(conf)
    //这里用java的synchronize进行了多线程并发访问的同步
    //因为task实际上是以java线程的方式，在一个CoarseGrainedExecutorBackend进程内并发运行
    //如果在执行业务逻辑的时候，要访问一些共享的资源，那么就可能出现多线程并发访问安全问题
    //所以在这里spark选择进行了多线程并发访问的同步
    synchronized {
      ...
      for ((name, timestamp) <- newFiles if currentFiles.getOrElse(name, -1L) < timestamp) {
        ...
        //通过utils的fetchFile方法，通过网络通信从远程拉取文件
        Utils.fetchFile(name, new File(SparkFiles.getRootDirectory), conf,
          env.securityManager, hadoopConf, timestamp, useCache = !isLocal)
          //进行内存缓存
        currentFiles(name) = timestamp
      }
      //遍历拉取的jar，并判断了一下时间戳，要去jar当前时间戳要小于目标时间戳
