...
/**
 * woeker中为application启动的executor，实际上是启动了这个coarsegrainedexecutorbackend进程
 */
private[spark] class CoarseGrainedExecutorBackend(
  ...
  //CoarseGrainedExecutorBackend被启动之后，就是执行这个prestart方法
   ...
   * 此时CoarseGrainedExecutorBackend，会创建executor对象，作为执行句柄
  ...
  override def receiveWithLogging = {
    ...
    case LaunchTask(data) =>
      if (executor == null) {
        ...
        //用内部的执行句柄，executor的launchtask方法，来启动一个task
