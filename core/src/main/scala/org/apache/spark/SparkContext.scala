...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  /**
   * SparkUI实在这里创建的，spark底层应用程序导入了jetty的工具类，实际上启动一个web服务器，默认绑定到4040端口上去，为我们提供一个web界面
   */
   ...
   */
  /**
   * 首先，hadoopfile方法的调用，会创建一个hadoopR调度，其中的元素就是<key,value>pair
   * key是hdfs或者本地文件的每一行的offset，value就是本行
   * 然后对hadoopRdd调用map方法，会提出key，只会保留value，然后获得一个map partitionRdd
  ...
  def runJob[T, U: ClassTag](
    ...
    //调用sparkContext之前初始化时候创建的DAGScheduler的runjob方法
...
object SparkContext extends Logging {
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      //这是我们常用的spark提价模式中的standalone方式，这种方式下如何进行taskscheduler的创建
      case SPARK_REGEX(sparkUrl) =>
        val scheduler = new TaskSchedulerImpl(sc)//创建TaskSchedulerImpl
        val masterUrls = sparkUrl.split(",").map("spark://" + _)//构建master的url
        val backend = new SparkDeploySchedulerBackend(scheduler, sc, masterUrls)//SparkDeploySchedulerBackend所属 的TaskSchedulerImpl传进来
        scheduler.initialize(backend)//至此，（当taskscheduler的调度池创建结束后）taskscheduler创建结束
        (backend, scheduler)
