...
abstract class RDD[T: ClassTag](
   ...
   * 那么就会通过cachemanager去获取数据，此时就会发现通过blockmanager获取不到数据，因为是第一次执行
   * 所以第一次还是会计算一次该rdd的数据，然后通过cachemanager的putinblockmanager()方法将器通过blockmanager进行持久化
   * rdd所在job运行结束了，然后启动单独job，进行checkpoint操作，此时又会执行到rdd的iterator方法
   * 那么，就会发现持久化级别不为空，默认从blockmanager直接读取持久化数据，正常情况下应该是可以的，如果是非正常情况下，持久化的数据丢失了
   * 这时候就会走else语句，调用computeOrReadCheckpoint方法，判断如果rdd是ischeckpoint为true，那么就会听他的父rdd的iterator方法
   * 其实就是从checkpoint的外部文件系统中读取数据
   */
