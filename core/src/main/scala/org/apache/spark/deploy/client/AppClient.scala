 ...
 * 它会接收一个spark master的url，以及一个applicationdescription，和一个集群事件的监听器
 * 以及各种事件发生时，监听器的回调函数
 * 相当于application和spark集群互相通信的组件
 */
/**
...
private[spark] class AppClient(
  ...
  class ClientActor extends Actor with ActorLogReceive with Logging {//这里是一个内部类，继承以及拥有一些trait（特性）
    ...
    override def preStart() {
      ...
      try {
        registerWithMaster()//注册到master中去
      } catch {
        case e: Exception =>
    ...
    def tryRegisterAllMasters() {
      for (masterAkkaUrl <- masterAkkaUrls) {
        ...
        val actor = context.actorSelection(masterAkkaUrl)//actor的引用
        actor ! RegisterApplication(appDescription)//将RegisterApplication(appDescription)发给master，application就会去master中注册
      }
    }

    def registerWithMaster() {
      tryRegisterAllMasters()//尝试连接所有的master，为什么会有多个master，看后面的注解，因为master支持多个节点，和两种机制的主备切换，相当于动态HA，动态切换
