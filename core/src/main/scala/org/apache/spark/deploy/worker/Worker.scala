...
private[spark] class Worker(
  ...
  override def receiveWithLogging = {
    ...
    case ExecutorStateChanged(appId, execId, state, message, exitStatus) =>
      //直接向master也发送一个ExecutorStateChanged
    ...
    case LaunchDriver(driverId, driverDesc) => {
      ...
      val driver = new DriverRunner(//上来就创建了driverrunner，这是一个线程
        conf,
        driverId,
        workDir,
        sparkHome,
        ...
        self,
        akkaUrl)
      drivers(driverId) = driver//缓存到内存缓存结构
      driver.start()//这个方法主要是创建了一个线程
    ...
    case DriverStateChanged(driverId, state, exception) => {
      ...
      //然后worker实际上会静driverstatechanged消息发送个master，master会进行状态改变处理
      master ! DriverStateChanged(driverId, state, exception)
      
      //将driver从本第缓存移除
      val driver = drivers.remove(driverId).get
      //将driver加入完成的driver队列中
