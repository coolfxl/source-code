...
/**
 * 管理一个driver的执行，包括在的driver失败的时候自动重启driver，目前这种方式仅仅使用于standalone集群部署模式
 */
/**
...
private[spark] class DriverRunner(
  ...
  def start() = {
    new Thread("DriverRunner for " + driverId) {//new 了一个java线程，Scala是运行在JVM中的
      override def run() {
        try {
          //创建driver的工作目录
          val driverDir = createWorkingDirectory()
          //下载用户上传的jar（如果是java，用maven打个包，如果是Scala则export jar
        ...
        //这个driverrunner这个线程，向他所属的worker的actor，发送一个driverstatechanged的事件
