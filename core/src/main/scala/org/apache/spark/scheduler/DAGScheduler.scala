...
/**
 * 实现了面向stage的调度机制的高层次的调度层，它会为每个job计算一个stage的DAG，追踪那个RDD和stage的输出是否被物化了
 * 物化就是说，写入磁盘或者内存，并且寻找一个最少（最小、最优）调度机制来运行job
 * 它会将stage作为taskset提交到taskschedulerImpl上，来在集群上运行他们（task）
 * 它除了处理stage的DAG，他还负责运行每个task的最佳位置，基于当前的缓存状态，将这些最佳位置提交给底层的taskschedulerimpl，此外，它还会处理由于shuffle输出文件丢失导致的失败
 * 在种情况下，旧的stage可能就会被重新提交，一个stage内部的失败，如果不是由于shuffle文件丢失所导致的（比如OOM），会被taskscheduler处理，他会多次重试每一个task
 * 直到最后，实在运行不下去了，就会取消整个stage
 */
/**
...
class DAGScheduler(
  ...
  /**
   * 获取某个stage的父stage，这里就涉及到了stage的划分算法
   * 这个方法的意思就说对一个stage
   * 如果他的最后一个rdd的所有依赖都是窄依赖，那么就不会创建任何新的stage
   * 但是，只要发现这个stage的rdd宽依赖了某个rdd，那么就用宽依赖的rdd,创建一个新的stage
   * 然后立即返回新的stage
   */
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    // We are manually maintaining a stack here to prevent StackOverflowError
    // caused by recursively visiting
    val waitingForVisit = new Stack[RDD[_]]//先入后出
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        visited += rdd
        if (getCacheLocs(rdd).contains(Nil)) {
           ...
           * 
           * 其实就是说：到了ShuffleRDD的时候，他会去判断当前这个ShuffleRDD是否是宽依赖，
           * 如果是，则将ShuffleRDD前面的rdd也就是MapPartitionsRDD进行stage的划分
           */
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
      ...
      properties: Properties = null)
  {
    //使用触发job的最后一个rdd，创建finalstage=newStage
    var finalStage: Stage = null
    try {
      // New stage creation may throw an exception if, for example, jobs are run on a
      // HadoopRDD whose underlying HDFS files have been deleted.
      //创建一个stage对象，加入DAGScheduler内部的缓存结构中
    ...
    if (finalStage != null) {
      //第二步：用finalstage创建一个job，就是说，这个job的最后一个stage，就是我们的finalstage
      ...
      if (shouldRunLocally) {
        ...
        listenerBus.post(
          ...
          //这个方法的调用会导致第一个stage的提交，并且导致其他的所有的stage，都给放入waitingstage队列里
          submitStage(finalStage)
          
          /**
           * 总结：stage划分算法实在太重要了，必须对stage划分算法很清晰，知道自己编写的spark application被划分为了几个job，
           * 每个job被划分为几个stage
           * 每个stage包括了哪些代码
           * 只有知道了每个stage包括了哪些代码之后
           * 在线上，如果发现某个stage执行特别慢，或者某个stage一直报错，
           * 这样才能对那个stage对应的代码进行排查，或者是性能调优
           * 
           * 1.从finalstage倒退
           * 2.通过款依赖，来进行新的stage的划分
           * 3.使用递归，优先提交父stage
           */
      }
    }
   ...
   * 提交stage的方法
   * 这个其实就是stage划分算法的入口，
   * 但是，stage划分算法，其实是有submitStage()方法与getMissingParentStages()方法共同构成的
   * 这样才能实现完整stage的划分
   */
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        //调用getMissingParentStages方法去获取当前这个stage的父stage
        val missing = getMissingParentStages(stage).sortBy(_.id)
        logDebug("missing: " + missing)
        //这里其实会反复递归调用，知道最初的stage，没有父stage
        //那么此时就会去首先提交这个第一个stage，stage0.
        //其余的stage，都在waitingstage里面
        if (missing == Nil) {
          //没有父stage
          ...
          //并且将当前stage，放入waitingStages等待执行的stage队列中
          waitingStages += stage
        }
      }
    } else {
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    //为stage创建指定数量的task
    /**
     * 这里很关键的一点是task的最佳位置的计算算法getPreferredLocs（）
     */
    val tasks: Seq[Task[_]] = if (stage.isShuffleMap) {
      partitionsToCompute.map { id =>
        ...
        //然后对finalstage之外的stage，他的isShuffleMap都是true
        //所以会创建ShuffleMapTask
        new ShuffleMapTask(stage.id, taskBinary, part, locs)
      }
    } else {
      //如果不是ShuffleMap,那么就是finalstage
      //finalstage就是创建resultTask的
      val job = stage.resultOfJob.get
      partitionsToCompute.map { id =>
    ...
    if (tasks.size > 0) {
      ...
      //最后，针对stage的task，创建taskset对象，调用taskscheduler的submittasks（）提交taskSet
      //默认情况下，standalone模式，使用的是taskSchedulerImpl,taskScheduler只是一个trait
   ...
   * 说白了，就是从stage的最后一个rdd开始，去找哪一个rdd的partition是被cache了，或者checkpoint了
   * 那么task的最佳位置，就是cache的或者checkpoint的partition的位置
   * 因为这样的话，task就在那个节点上执行，不需要重新计算之前的rdd了
   */
    ...
    : Seq[TaskLocation] =
  {
    ...
    //如果这个stage从最后一个rdd到最开始的rdd，partition都没有被缓存或者checkpoint，那么task的最佳位置就是Nil，就是没有，所以要重新计算
    Nil
  }
