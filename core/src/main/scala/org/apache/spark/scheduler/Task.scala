...
private[spark] abstract class Task[T](val stageId: Int, var partitionId: Int) extends Serializable {
  ...
  final def run(taskAttemptId: Long, attemptNumber: Int): T = {
    ...
    //创建一个TaskContextImpl也就是task的执行上下文，里面记录了task的一些全局性的数据
    //比如task重试了几次，属于哪个stage，处理的是RDD的哪个partition
  ...
  /**
   * 调用到了抽象方法，意味着这个类，只是一个模板类，或者抽象父类
   * 仅仅封装了一些类通用的数据和操作
   * 而关键的操作，全部都依赖于子类的实现
   * task的子类，都有哪些实现
   * 有shuffleMapTask和RresultTask这两种实现，要运行他们的runtassk方法，才能执行我们自定义的算子和逻辑
   */
