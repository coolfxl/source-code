  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverActor(sparkProperties: Seq[(String, String)]) extends Actor with ActorLogReceive {
    ...
    def makeOffers() {
      ...
      //第二步：分配哈task到executor之后，执行自己的launchtasks方法，将分配的task发送launchtask消息
      //到对应的executor上去，由executor启动并执行task
      
      //给resourceoffers方法传入的是这个application所有可用的executor，并且将其封装成了workeroffer，每个workeroffer代表了
    ...
    def launchTasks(tasks: Seq[Seq[TaskDescription]]) {
      for (task <- tasks.flatten) {
        //首先将每个executor要执行task信息，进行序列化成一个二进制字节数组
