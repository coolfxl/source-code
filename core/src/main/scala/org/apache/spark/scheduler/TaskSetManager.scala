 ...
 * 会负责重试task，知道超过重试的次数限制，并且会通过延迟调度，为这个taskset处理本度化调度机制，他的主要接口是resourceOffer
 * 在这个接口中，taskset会希望在一个节点上运行一个任务，并且接收任务的状态改变消息，来知道他负责的task的状态改变了
 */
/**
...
private[spark] class TaskSetManager(
  ...
  /**
   * 这个方法大致的意思就是
   * 会去判断这个executor在这个本地化级别，之前的等待时间是多少
